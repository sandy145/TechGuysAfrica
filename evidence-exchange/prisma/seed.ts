 ...
 * Seed data.
 *
 * The scenario is deliberately the one this product was built for: a full
 * inspection whose exit conference has happened, whose evidence window is
 * running, and which contains a finding where the provider has already sent
 * the document that resolves it — sitting unreviewed. On the licensor's
 * dashboard that finding is the loudest thing on the screen, which is the
 * entire difference between this and an inbox.
 *
...
const DEMO_PASSWORD = "Exchange2026!";
...
async function main() {
  ...
  console.log("  admin@example.wa.gov        Priya Raman, program administrator");
